def gen_ai(**kwargs):
    ...
    guard = Guard.use([
    RestrictToTopic(valid_topics=["tabular data", "summary", "table"], invalid_topics=['general'])
])
    ...
    try: 
      ...
      chain = prompt | llm | guard.to_runnable() | StrOutputParser()
      

      result = chain.invoke({"column_context": str(column_dict), "context_feedback":str(feedback_similar_data), "context_data": str(table_similar_data),"input": query})
      return result
...
response = gen_ai(**{'Region': df['Region'], 'Product': df['Product'], 
